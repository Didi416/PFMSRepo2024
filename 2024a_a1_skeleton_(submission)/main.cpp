...
int main(int argc, char *argv[]) {
    ...
    pfms::nav_msgs::Odometry origin = controllers.at(0)->getOdometry();
...
//     pfmsConnectorPtr->read(origin,controllers.at(0)->getPlatformType());
//     std::cout <<
//                 origin.time << " " <<
//                 origin.position.x << " " <<
//                 origin.position.y << " " <<
//                 origin.yaw << " " <<
//                 origin.linear.x << " " <<
//                 origin.linear.y << std::endl;

    // double distance;
    // double time;
    // pfms::nav_msgs::Odometry estimatedGoalPose;

    // bool reachable = controllers.at(0)->setGoal(goals.at(0));
    // bool checked = controllers.at(0)->checkOriginToDestination(origin, goals.at(0), distance, time, estimatedGoalPose);
