...
Ackerman::Ackerman(){
    ...
    // reachGoalsTimer_ = this->create_wall_timer(std::chrono::milliseconds(10), std::bind(&Ackerman::timerCallback, this));
...
void Ackerman::reachGoals(void){
    ...
    double steeringPH; //placeholder for steering for when we need to calculate distance (computeSteering function in Audi library)
    Audi audi;
    for (size_t i=0; i<pfmsGoals_.size(); i++){
        goalReached = false;
        throttle_.data = 0.25;
        state = 1;
...
void Ackerman::threadFunction(){
    //This is an example of logging, this will be called every 10ms
    while(rclcpp::ok()){
        // RCLCPP_INFO(this->get_logger(), "Thread function example");
