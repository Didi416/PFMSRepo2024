...
protected:
  ...
  pfms::nav_msgs::Odometry estimatedGoalPose_; //!<or checkOriginToDestination function input and setting goals, the estimated goal pose is required to be used as the next origin pose
  //distance and time to current goal, constantly updated through reachGoals calling checkOriginToDestination
  double distanceToCurrentGoal_; //!<distance from platform to current goal
  double timetoCurrentGoal_; //!<time from platform to current goal
  //total distance and time travelled from start to current time, recalculated throughout reachGoals
  double distanceTravelled_; //!< total distance travelled up to current time
  double timeTravelled_; //!< total time in motion up to current time
