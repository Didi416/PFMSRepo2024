...
void Mission::setGoals(std::vector<pfms::geometry_msgs::Point> goals, pfms::PlatformType platform){
    ...
    std::vector<pfms::geometry_msgs::Point> tempGoals; //temporary goals vector to store the correct order of goals before transferring to missionGoals_
    TSP tsp; //create TSP object for graph generation and searching
    std::vector<tsp::Point> goalPoints(missionGoals_.size(),{0,0,0}); //to copy the pfms::geometry_msgs::Points to tsp::points for tsp to access and keep tsp library completely independant
    ...
    switch (objective_){ //check mission objective (BASIC, ADVANCED or SUPER)
        ...
        case mission::Objective::ADVANCED: //for ADVANCED mode
            ...
            for (int j=0; j<missionGoals_.size(); j++){ //iterate through goals
                ...
                controllers_.at(a)->checkOriginToDestination(origin, missionGoals_.at(j), distancesFromOrigin_.at(j), time, estimatedGoalPose);
            }
            for (int i=0; i<missionGoals_.size(); i++){ //iterate through goals
                //Copy goal points from missionGoals into the tsp Point struct
                goalPoints.at(i).x = missionGoals_.at(i).x;
                goalPoints.at(i).y = missionGoals_.at(i).y;
                goalPoints.at(i).z = missionGoals_.at(i).z;
            }
            graph = tsp.generateGraph(goalPoints);//generate a graph of the goals, to determine whcih goals can be accessed from other goals
